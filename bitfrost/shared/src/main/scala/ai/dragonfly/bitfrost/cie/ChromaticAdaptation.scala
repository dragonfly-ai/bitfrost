...
object ChromaticAdaptation {
  ...
  lazy val Bradford: Matrix = new Matrix(
    MatrixValues(
      VectorValues(0.8951, 0.2664, -0.1614),
      VectorValues(-0.7502, 1.7135, 0.0367),
      VectorValues(0.0389, -0.0685, 1.0296)
    )
  )

  lazy val Bradford_Inverse: Matrix = new Matrix(
    MatrixValues(
      VectorValues(0.9869929, -0.1470543, 0.1599627),
      VectorValues(0.4323053, 0.5183603, 0.0492912),
      VectorValues(-0.0085287, 0.0400428, 0.9684867)
    )
  )

  lazy val Von_Kries: Matrix = new Matrix(
    MatrixValues(
      VectorValues(0.40024, 0.7076, -0.08081),
      VectorValues(-0.2263, 1.16532, 0.0457),
      VectorValues(0.0, 0.0, 0.91822)
    )
  )
...
case class ChromaticAdaptation[S <: WorkingSpace, T <: WorkingSpace](source:S, target:T, m:Matrix = Bradford) {
  ...
  val s:VectorValues = (m * source.illuminant.asColumnMatrix).getRowPackedCopy()

  val t:VectorValues = (m * target.illuminant.asColumnMatrix).getRowPackedCopy()

  val M:Matrix = m.inverse().times(new Matrix(
    MatrixValues(
