...
object PLY {
  ...
  def write(volumeMesh: VolumeMesh, transform: Vector3 => ARGB32, out: java.io.OutputStream): Unit = {
...
element vertex ${volumeMesh.vertices.length}
property float x
property float y
property float z
property uchar red
...
element face ${volumeMesh.triangles.length}
property list uchar uint vertex_indices
end_header
"""
    )

    for (i <- volumeMesh.vertices.indices) {
      val v:Vector3 = volumeMesh.vertices(i)
      val c:ARGB32 = transform(v)
      sout.write(s"${v.x} ${v.y} ${v.z} ${c.red} ${c.green} ${c.blue} ${c.alpha}\n")
    }

    for (triangle <- volumeMesh.triangles) {
